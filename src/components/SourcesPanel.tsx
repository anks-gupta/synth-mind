...
import { SourceItem, SourceType, Citation } from '@/lib/types';
import {
  FileText,
  Video,
  Globe,
...
export function SourcesPanel({
  ...
  const panelContent = (
    <aside className="w-80 max-w-[85vw] lg:max-w-none border-r border-[#1e293b] bg-[#0f172a] flex flex-col h-full shrink-0 overflow-hidden">
      ...
      {sources.length > 0 && (
        <div className="px-4 py-2 border-t border-b border-[#1e293b] flex items-center justify-between text-xs text-slate-400 bg-[#070b12]/60">
          ...
          <button
            ...
            className={`flex items-center space-x-1 px-2.5 py-1 rounded-lg text-[11px] font-bold transition-all cursor-pointer ${allSelected
      ...
      {/* Source Cards List */}
      <div className="flex-1 overflow-y-auto min-h-0 touch-pan-y overscroll-contain p-4 space-y-3">
        {isLoadingSources ? (
          ...
          sources.map((source) => {
            ...
            return (
              <div
                ...
                className={`px-3 py-2.5 rounded-xl border transition-all cursor-pointer group ${isError
              ...
              >
                <div className="flex items-center justify-between gap-2.5">
                  <div className="flex items-center space-x-2.5 min-w-0 flex-1">
                    {/* Selection Checkbox */}
                    <div
                      className={`w-4 h-4 rounded flex items-center justify-center transition-all border shrink-0 ${isChecked
                        ? isError
                    ...
                    <div className="min-w-0 flex-1">
                      <span
                        className={`text-xs font-semibold truncate block leading-snug ${isChecked ? 'text-slate-100' : 'text-slate-300 group-hover:text-slate-100'
                          }`}
                  ...
                  <div className="relative shrink-0">
                    ...
                    {activeMenuSourceId === source.id && (
                      <div
                        className="absolute right-0 top-8 w-44 rounded-xl bg-[#0f172a] border border-[#1e293b] shadow-2xl p-1 z-50 animate-in fade-in-0 slide-in-from-top-1 duration-150 space-y-0.5"
                      ...
                      >
                          ...
                          <button
                            ...
                            onClick={() => {
                              ...
                              onSelectCitation({
                                ...
                                textSnippet: source.content || 'Extracted source text ready for grounded RAG research.',
                                urlOrPath: source.urlOrPath,
                              });
                            }}
                            className="w-full flex items-center space-x-2 px-3 py-2 rounded-lg text-xs font-semibold text-violet-300 hover:bg-[#1e293b] transition-colors cursor-pointer"
                          >
                        ...
                        {source.urlOrPath && (source.type === 'youtube' || source.type === 'url' || source.type === 'pdf') && (
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            onClick={() => setActiveMenuSourceId(null)}
                            className="w-full flex items-center space-x-2 px-3 py-2 rounded-lg text-xs font-semibold text-slate-300 hover:text-cyan-300 hover:bg-[#1e293b] transition-colors"
                          >
      ...
      {isAddModalOpen && (
        <div className="fixed inset-0 bg-[#070b12]/80 backdrop-blur-md flex items-center justify-center p-3 sm:p-4 z-50 animate-in fade-in-0 duration-150 select-text">
          <div className="w-full max-w-[95vw] sm:max-w-lg bg-[#0f172a] border border-[#1e293b] p-4 sm:p-6 rounded-2xl shadow-2xl space-y-4 my-auto relative animate-in zoom-in-95 duration-150 max-h-[90vh] overflow-y-auto">
              ...
              {(['youtube', 'pdf', 'url', 'vtt', 'text'] as SourceType[]).map((t) => {
                ...
                return (
                  <button
                    ...
                    className={`py-2 rounded-xl text-xs font-semibold flex flex-col items-center space-y-1 transition-all border ${isActive

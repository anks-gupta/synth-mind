...
export function SourceViewer({ citation, onClose }: SourceViewerProps) {
  ...
  return (
    <div className="border-t border-[#1e2942] bg-[#0e1526] p-4 relative z-30 shadow-2xl animate-in slide-in-from-bottom-2 duration-200 max-h-[60vh] sm:max-h-[70vh] overflow-y-auto">
      <div className="flex flex-wrap items-center justify-between gap-2.5 mb-3 border-b border-[#1e2942] pb-2.5">
        <div className="flex items-center space-x-2 min-w-0 flex-1 sm:flex-initial">
          ...
          <span className="text-xs font-bold text-slate-100 truncate max-w-[160px] xs:max-w-[220px] sm:max-w-xs md:max-w-md" title={citation.sourceTitle}>
            {citation.sourceTitle}
          </span>
          <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-sky-500/10 text-sky-400 border border-sky-500/20 font-semibold whitespace-nowrap shrink-0">
            {isYouTube ? 'YouTube' : isPdf ? 'PDF Reference' : isVtt ? 'Transcript' : isText ? 'Note' : 'Web Article'}
          </span>
        </div>
        ...
        <div className="flex items-center space-x-2 shrink-0 ml-auto">
          {isYouTube && directYouTubeUrl ? (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="px-2.5 py-1 rounded-lg bg-rose-500/10 hover:bg-rose-500/20 border border-rose-500/30 text-rose-300 text-xs font-bold flex items-center space-x-1.5 transition-all hover:scale-105 whitespace-nowrap"
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="px-2.5 py-1 rounded-lg bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/30 text-emerald-300 text-xs font-bold flex items-center space-x-1.5 transition-all hover:scale-105 whitespace-nowrap"
              title="Open PDF at page"
            >
              <FileText className="w-3.5 h-3.5" />
              <span>PDF (Page {citation.pageNumber || 1})</span>
              <ExternalLink className="w-3 h-3" />
            </a>
          ) : isWeb && isExternalUrl ? (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="px-2.5 py-1 rounded-lg bg-cyan-500/10 hover:bg-cyan-500/20 border border-cyan-500/30 text-cyan-300 text-xs font-bold flex items-center space-x-1.5 transition-all hover:scale-105 whitespace-nowrap"
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="px-2.5 py-1 rounded-lg bg-violet-500/10 hover:bg-violet-500/20 border border-violet-500/30 text-violet-300 text-xs font-bold flex items-center space-x-1.5 transition-all hover:scale-105 whitespace-nowrap"
          ...
          <button
            onClick={onClose}
            className="text-slate-400 hover:text-white p-1 rounded-lg hover:bg-[#1e2942] transition-colors cursor-pointer shrink-0 ml-1"
            title="Close viewer"
          >
            <X className="w-4 h-4" />
          </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="aspect-video bg-[#090d16] rounded-xl border border-[#1e2942] flex items-center justify-center overflow-hidden relative group">
          {isYouTube && videoId ? (
            <>
              {isIframeLoading && (
                <div className="absolute inset-0 bg-[#090d16] flex flex-col items-center justify-center space-y-2 z-10 pointer-events-none">
              ...
              <iframe
                className={`w-full h-full relative z-0 transition-opacity duration-300 ${isIframeLoading ? 'opacity-0' : 'opacity-100'}`}
                src={`https://www.youtube-nocookie.com/embed/${videoId}?start=${citation.startTime || 0}&autoplay=1&playsinline=1&controls=1&enablejsapi=1&rel=0`}
